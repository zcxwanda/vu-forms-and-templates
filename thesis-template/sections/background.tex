...
However, similarity in topic does not necessarily mean that an LLM behaves like a human patient. In a direct comparison using the same motivational-interviewing profiles, LLM-simulated and human patients discussed similar content but differed in how they expressed their responses~\cite{hoang2026standardised}. This distinction is relevant here because the experiment evaluates controlled response patterns rather than claiming that the generated conversations reproduce the full range of real patient behavior.

User simulation is also used to evaluate task-oriented dialogue systems. Interactive evaluation has been proposed because a fixed transcript cannot reflect how each system response changes the user's next turn~\cite{cheng2022interactive}. RADDLE evaluates robustness to language variation, speech errors, unseen entities, and out-of-domain utterances, and reports weaknesses in existing task-oriented dialogue systems~\cite{peng2021raddle}. These studies show that simulated user behavior can reveal weaknesses that may not appear in standard interactions.

This study applies that idea to questionnaire-based mental health assessment. It varies the simulated patient's cooperation while keeping the clinical case and questionnaire fixed, allowing the resulting dialogue and diagnosis to be compared under controlled conditions.

\subsection{Clinical Profiles and Grounding}

The patient agent responds according to a clinical profile that specifies a target disorder, relevant symptoms, and background information. The profile provides the reference case for both patient generation and diagnostic evaluation. Patient answers should therefore remain consistent with the profile even when resistance changes how the information is presented.

Profile adherence matters because unsupported symptoms can alter the diagnostic evidence. Roleplay-doh found that initial GPT-4 responses failed to satisfy expert-defined patient principles or dialogue conventions in 20\% of cases~\cite{louie2024roleplaydoh}. Related role-play research has also shown that model knowledge can lead a simulated character to answer outside the information assigned to that character~\cite{sadeq2024rolefact}. In this study, unsupported additions would make it unclear whether a diagnostic change was caused by resistance or by a change in the simulated case. Section~\ref{s:methodology} describes the prompt boundaries and truth maps used to reduce this problem.
...
In this thesis, resistance is identified from the patient's response rather than inferred from the patient's motive. It includes responses that make the requested information less available, reframe a problem supported by the clinical profile, or disrupt the questionnaire process. This definition is closest to the event-level and response-based approaches above, but the categories are reorganized around their effect on diagnostic information collection. Similar responses in real assessments may result from symptoms, anxiety, fatigue, misunderstanding, or difficulty expressing feelings. The resistance label is therefore used as an experimental condition, not as a clinical explanation of the patient. 
