...
The study uses synthetic conversations rather than interactions with real clients. Its repeated runs reuse the same five profiles and condition prompts, so they represent repeated model generations rather than independent patient cases. Gemini 2.5 Flash is used for all three agents and most evaluation; some findings may therefore be model-specific, and using the same model for generation and scoring may introduce bias. Generalization requires testing other models and assessment settings, as well as clinical review of the resistance taxonomy.

Intensity is implemented mainly through activation frequency. Expression levels are used to reduce repetitive wording, with a small intensity-dependent sampling preference. Low, medium, and high use probabilistic activation, whereas extreme activates every eligible turn and always uses Level 3 guidance. Extreme is therefore not simply a higher value on the same probability range. Future work could add probabilistic conditions between high and extreme to test whether diagnostic accuracy declines gradually or only when resistance appears on almost every turn. Reframing introduces another difference: its truth map limits activation to profile-positive turns, while responses on other turns may still disclose profile information. Future work should evaluate reframing separately by reporting how often it appeared on profile-positive turns and how much clinical information was disclosed on other turns.
