...
\begin{table*}[tb]
    ...
    \begin{tabular}{p{0.22\linewidth}p{0.70\linewidth}}
        ...
        Resistance sub-category &
        A resistance sub-category is a more specific form of resistance within a category. Sub-categories describe concrete response types used for resistance profile construction, simulation design, and qualitative interpretation. \\

        Resistance intensity &
        Resistance intensity describes how often resistance appears in the simulated client's responses. It is an experimental condition rather than a taxonomy category. \\
...
\begin{table*}[tb]
    ...
    \begin{tabular}{p{0.15\linewidth}p{0.19\linewidth}p{0.25\linewidth}p{0.17\linewidth}p{0.20\linewidth}}
        \toprule
        \thead{Category} & \thead{Sub-category} & \thead{Definition} & \thead{Example} & \thead{Boundary rule} \\
        \midrule
...
Resistance intensity controls the expected frequency of activated resistance turns. When a turn is activated, an expression level is also sampled. Expression level changes how strongly the selected sub-category appears in that response, but it is not treated as a separate experimental condition. The main settings used in the current implementation are shown in Table~\ref{tab:activation-settings}.

\begin{table*}[tb]
    \centering
    \small
    \tabcap{Turn-level activation and expression settings.}
    \label{tab:activation-settings}
    \begin{tabular}{p{0.18\linewidth}p{0.30\linewidth}p{0.40\linewidth}}
        \toprule
        \thead{Intensity} & \thead{Target activation range} & \thead{Expression-level probabilities} \\
        \midrule
        Low & 0.15--0.30 & Level 1: 0.44; Level 2: 0.28; Level 3: 0.28 \\
        Medium & 0.40--0.60 & Level 1: 0.28; Level 2: 0.44; Level 3: 0.28 \\
        High & 0.70--0.85 & Level 1: 0.28; Level 2: 0.28; Level 3: 0.44 \\
        \bottomrule
    \end{tabular}
\end{table*}

For non-reframing categories, activation can be considered on every ordinary patient-answer turn. For reframing categories, activation is limited by the truth map. This means that a high-intensity reframing run can still have fewer activated turns than a high-intensity non-reframing run if the clinical profile has fewer positive questionnaire items. A cooldown rule is also used to reduce unnatural back-to-back activations, so the observed activation rate may not exactly match the sampled target rate in every conversation.
...
\begin{table*}[tb]
    ...
    \begin{tabular}{p{0.20\linewidth}p{0.30\linewidth}p{0.40\linewidth}}
        \toprule
        \thead{Sub-category} & \thead{Assistant question} & \thead{Patient response excerpt} \\
        \midrule
